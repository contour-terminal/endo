...
| Multi-process pipes | ✅ |
| Builtins: `exit`, `true`, `false`, `read` (-p/-r/-s/-n/-t/-d, IFS splitting), `cd` (incl. `cd -`), `pwd`, `set`, `unset`, `export`, `bind`, `echo`, `which`, `cat` (-n/-b/-s/-E/-T/-A/-r/--range/-c/--columns/-R/--rows/--raw, markdown document rendering with tables, OSC-8 links, double-width/height titles, centered HTML blocks and inline Sixel images; standalone image rendering via Sixel; binary data detection), `sleep`, `fetch`, `rm` (-r/-R/-f/-d/-v/-i/--recursive/--force/--dir/--verbose/--help/--), `mkdir` (-p/-v/--parents/--verbose/--help/--), `cp` (-r/-R/-f/-n/-v/--recursive/--force/--no-clobber/--verbose/--help/--), `whoami`, `hostname`, `date` (--epoch/--iso/--utc/+FORMAT/-d), `cal` (-3/-y/-m/-s/--no-color, month/year and year-only forms, colorized today/weekend highlighting), `uname` (-s/-n/-r/-m/-a), `basename`, `dirname`, `realpath`, `touch` (-c), `ln` (-s/-f/-v), `mktemp` (-d/-p), `head` (-n), `tail` (-n/-f), `wc` (-l/-w/-c), `sort` (-r/-n/-u/-k), `uniq` (-c/-d/-i), `cut` (-d/-f/-c), `tr` (-d/-s), `tee` (-a), `pgrep` (-f/-x/-i/-v/-c/-l/-n/-o/-u/-d), `pidof` (-s/-q/-S/-d/-o) | ✅ |
...
- [x] Parser multi-line continuation for F# operators
  - [x] `|>` pipeline operator across line breaks in `parseFSharpPipeline()` — `consumeUntilNotOneOf(Token::LineFeed)` with pushback
  - [x] `@` list concat and `::` cons operators across line breaks in `parseFSharpCons()` — same pattern
...
- [x] F#-style `<>` not-equal operator (alternative to `!=`, both supported)
  ...
  - [x] 2 test cases (integer comparison, string comparison)
- [x] Source formatter string escape handling
  - [x] `escapeString()` helper re-escapes control characters (`\n`, `\t`, `\r`, `\0`, `\a`, `\b`, `\f`, `\v`), backslash, double quote
  - [x] Non-printable characters (<0x20 or 0x7F) escaped as `\xHH` hex notation
  - [x] Applied to `LiteralExpr`, `FStringExpr`, and `LiteralPattern` visitor methods
  - [x] 4 formatter tests (newline, tab, backslash, idempotency)
- [x] Source formatter literal preservation
  - [x] Integer literal base preservation: `0xFF`, `0b1010`, `0o755` preserved via `IntLiteralExpr::originalText`
  - [x] Float literal notation preservation: `1e10`, `2.5e2` preserved via `FloatLiteralExpr::originalText`
  - [x] Unit parameter `()` preserved in `let f ()` and `fun () ->` (was `(_: unit)`)
...
`Size` builtin record type added with human-readable display (e.g., "1.5 KB"). Size literals (`1_B`, `1_KB`, `1_MB`, `1_GB`, `1_TB`) and module constructors (`Size.fromBytes`, `Size.fromKB`, `Size.fromMB`, `Size.fromGB`, `Size.fromTB`) supported. `ls` file size is now a `Size` object. Size values support comparison operators and `.bytes` field access. IDE completions and hover documentation included. Table formatter now renders `Size` objects as human-readable strings (e.g., "4 KB") instead of raw pointer values. Size formatting extracted into shared `builtins::formatSizeToString()` utility.
Built-in record types (`FileInfo`, `ProcessInfo`, `JobInfo`, `Size`, `DateTime`, `FileMode`) fully documented in `standard-library.md` (sections 15.13--15.19) and `type-system.md` (section 3.7). LSP hover documentation added for all record types and shell data commands (`ls`, `ps`, `jobs`), formatting helpers (`formatMode`, `formatDateTime`, `toText`), permission tests (`isReadable`, `isWritable`, `isExecutable`), and `fetch`/`which`/`rand`. Fixed `FileInfo.size` completion metadata from `"int"` to `"Size"` and added `Size` to `recordTypeFields` enabling nested `f.size.bytes` dot completion.
`FileMode` builtin record type added wrapping raw permission bits with human-readable `rwxr-xr-x` display. `FileMode.fromBits` constructor, computed properties (`isReadable`, `isWritable`, `isExecutable`, `owner`, `group`, `other`), and `.bits` field access. `ls` mode field is now a `FileMode` object. Existing `formatMode`/`isReadable`/`isWritable`/`isExecutable` updated for dual-mode compatibility (accept both raw int and FileMode). Table formatter renders FileMode as permission string. Size columns right-aligned in table output.
`TimeSpan` builtin record type added for type-safe duration handling. Single `milliseconds` field with constructors (`TimeSpan.fromMilliseconds`, `TimeSpan.fromSeconds`, `TimeSpan.fromMinutes`, `TimeSpan.fromHours`, `TimeSpan.fromDays`). Human-readable display formatting (e.g., "1d 2h 30m 15s 100ms"). F# `sleep` function accepts TimeSpan objects (`sleep (TimeSpan.fromSeconds 5)` or `TimeSpan.fromSeconds 5 |> sleep`). `formatTimeSpan` converts TimeSpan to string. Shell `sleep` builtin unchanged. IDE completion, hover documentation, and type registry integration included.
`Markdown` builtin product type added for first-class markdown handling. `markdown "# Hello"` constructor creates a Markdown object. Module-qualified methods: `Markdown.render` (terminal rendering via `tui::MarkdownRenderer` when TTY, raw text fallback), `Markdown.toHtml` (line-by-line HTML conversion with headings, bold, italic, code, fenced blocks, blockquotes, lists), `Markdown.toText` (strips formatting characters), `Markdown.content` (raw markdown string). Pipeline support (`"# text" |> markdown |> Markdown.toText`). Auto-display renders markdown with terminal formatting. `string md` returns raw content. WASM playground and test runtime include mock `markdown_render` callbacks.
Auto-display of F# expression results in interactive REPL extended to all expression forms: function calls (`add 3 4`), `match`, `if-then-else`, `rand`, `which`, variadic functions, and command substitution `$()` now auto-display their results at the shell prompt. String results are quoted (`"hello"`). Parser `_autoDisplay` flag ensures auto-display only activates in interactive mode (not script execution). `isUnitProducingExpr()` suppresses display for unit-producing calls like `print`.
Generic type-aware completion system: all type/builtin/property metadata now derived from authoritative registries (`TypeRegistry`, `BuiltinSignatures`), eliminating duplication across shell, LSP, and diagnostics. `TypeRegistryCompletionAdapter` bridges `TypeRegistry` to the completion system. Module functions (`Option.map`, `DateTime.now`, `Size.fromBytes`, etc.) registered in `TypeDescriptor::moduleFunctions` and used generically in `dotAccessCandidates()`. `BuiltinInfo` struct with `userFacingBuiltins()` replaces hard-coded builtin/property lists in `CompletionCandidates.cpp`, `DiagnosticsCollector.cpp`, and `CommandResolver.cpp`. `NativeProperty` carries a `description()` for runtime-driven property completion. LSP now sees builtin types (`ProcessInfo`, `FileInfo`, `DateTime`, `Size`, `FileMode`) for dot-access completion.
Colored file listing output with Nerd Font icons added to `ls` table display (TTY only). File names colored by type: directories (bold blue), executables (bold green), hidden files (dim), source files by language (67 extension mappings). Data-driven `FileTypeStyle` module with `constexpr` sorted extension table and binary search lookup. `shell_ls_icons` boolean property (default: `true`) configurable via `init.endo` to disable icons. LSP hover/completion/diagnostics support for the new property.
Single Source of Truth refactor: `PropertyDescriptors` (58 properties with enum values, types, descriptions, detail docs) and `StdlibDescriptors` (50+ stdlib functions with user-facing names, VM names, arity, callbacks, descriptions, detail docs) centralize all metadata. `BuiltinSignatures.cpp` registration, `CompletionCandidates.cpp` completion data, `BuiltinImpls.cpp` `resolveSharedImpl()`, and `userFacingBuiltins()` all derive from these tables. Shell/keyword builtin descriptors also table-driven. Adding a new property or stdlib function now requires editing only one descriptor entry.
Unified Builtin Registry: `StdlibDescriptor` extended with `ParamDescriptor` (param name + `LiteralType`) and `returnType`, replacing the `arity` field. `NativeCallback::param(LiteralType, string)` added for data-driven VM registration. `registerFSharpBuiltins()` body replaced with a 10-line loop over `stdlibDescriptors()`, eliminating ~330 lines of hand-coded fluent API calls. `Shell::registerLanguageBuiltins()` body replaced with a single `registerFSharpBuiltins()` call using `resolveSharedImpl` as the callback resolver, eliminating ~260 lines of duplicate registration. Adding a new F# builtin now requires only a single descriptor entry with param types, return type, callback, and documentation.
Nested underscore dot-access completion: `ls |> map _.mode.` now shows only `FileMode` fields (`bits`) instead of all record fields. Pipeline type resolution extended from `_` to `_.` prefixes in both shell (`FSharpCompleter`) and LSP (`Completer`). `CompletionCandidates::dotAccessCandidates()` resolves `_` via `pipelineElementType` in the nested dot-access branch. Command-to-type mappings (`ls→FileInfo`, `ps→ProcessInfo`, `jobs→JobInfo`) moved from hardcoded Shell.cpp entries to data-driven `TypeDescriptor::producingCommand` field with `builtinCommandOutputTypes()` adapter. `CompletionDataSource` extended with `commandOutputTypes` for LSP support.
Annotation propagation hardening: centralized all 5 type annotation maps (innerType, objectTypeId, innerObjectTypeId, listElementTypeId, listElementLiteralType) into single `propagateAllAnnotations()` helper, replacing 7 manual propagation sites. Fixed 3 partial propagation sites (match result storage 2/5→5/5, match result load 2/5→5/5, HOF param binding 3/5→5/5, constructor pattern binding 1/5→5/5). Added cons/list pattern element type annotation so head elements from pattern matching on typed lists receive correct innerType. Replaced both `convertToString()` N2S fallback paths (Object and Void type) with `object_to_string` runtime dispatch as safety net — values with missing annotations now use `valueToString()` runtime type identification instead of blindly reinterpreting string pointers as integers. Eliminates the entire class of raw-pointer-printing bugs (e.g., `135655133330448` instead of `"hostname"`).
Source code formatter: `endo format` CLI subcommand and LSP `textDocument/formatting` support. Trivia-based formatting — lexer extended with opt-in comment collection (`CommentStyle::Shell`, `CStyle`, `FSharp`), comments interleaved at original positions during output. `SourceFormatter` implements full `ast::Visitor` + `pattern::PatternVisitor` (compiler-enforced completeness). `.endo-format` YAML config files with `indentWidth`, `useSpaces`, `maxLineWidth`, `trailingNewline`, `blankLinesBetweenTopLevel`. CLI options: `--check` (exit 1 if unformatted), `--diff`, `--stdout`, `--config PATH`, stdin support. LSP returns single `TextEdit` covering entire document. Parse failures return original source unchanged. Idempotent formatting verified by tests. One-expression-per-line rule: `IfExpr` with else never puts all three expressions on one line (uses compact two-line or multi-line with symmetry enforcement); `MatchExpr`/`TryWithExpr` arm bodies indent on next line when compound; `TryFinallyExpr` uses compact two-line or multi-line format; `LambdaExpr` and `LetInExpr` indent body on next line when compound. `isCompoundExpr()` helper identifies expressions too complex for inline placement (IfExpr, MatchExpr, BlockExpr, LambdaExpr, LetInExpr, TryWithExpr, TryFinallyExpr), unwrapping ParenExpr wrappers. Fix: `ForInStmt` and `WhileStmt` now have `location` set by parser, preventing leading comments from being reordered inside loop bodies. Defensive `findFirstLine()` fallbacks added for both node types. Fix: comment collection now uses `unicode::to_utf8()` instead of `static_cast<char>()`, preserving multi-byte Unicode characters (e.g., em-dash U+2014) in comments. User-authored blank line preservation: blank lines between statement groups in the original source are preserved at any indent level, with multiple consecutive blank lines normalized to exactly one. Pre-computed blank line set from source scanned at format time; `hasBlankLineBetween()` O(log n) lookup.
`lines` string builtin: `lines text -> list<string>` splits text by newline characters (`\n`), stripping `\r` for cross-platform `\r\n` support. Trailing newline does not produce an empty element. Available in both application context (`lines text`) and pipeline context (`text |> lines`). Registered in BuiltinDescriptors, StdlibDescriptors, IRGenerator (application + pipeline), with full LSP/completion support.
Comprehensive formatter bug fix pass: 13 bugs fixed across formatter, parser, and type system ensuring all 1429 test files survive format round-trip with zero failures and full idempotency. New AST nodes: `CompositionExpr` (preserves `>>` / `<<` compose operators instead of desugaring to lambdas) and `PlaceholderLambdaExpr` (preserves `_` placeholder syntax instead of desugaring to `fun __x -> ...`). Formatter fixes: float literals retain decimal point (`1.0` not `1`), variadic `...args` prefix preserved, `StructuredPipelineSourceExpr` no longer wraps in invalid `$(...)`, union type indentation and named fields, property get/set multi-line bodies, union constructor tuple parens, `DataSourceExpr` pipe-chain ordering, `ShellCommandExpr` leading comments, f-string literal brace escaping (`{{`/`}}`). Parser fixes: f-string single-literal optimization removed (preserves `$"..."`), `consumeNewlines()` added after `try`/`finally` keywords, all 11 statement-level `|>` continuation loops handle newlines with pushback. Type system fixes: `PrimitiveType::Str` prints as `string` (user syntax), nested generic types space closing `>` to avoid `>>` ambiguity.
CLI `--key=value` syntax generalized: `consumeOptionValue()` helper supports both `--key=value` and `--key value` forms for all value-taking options (`--module-path`, `--log-file`, `--log`, `--agent-trace`). Previously `--module-path` only accepted space-separated form.
Module resolution fix for script execution: `sourceFilePath` now set on `FSharpPersistentState` when running scripts via CLI (`executeScript`), `source` builtin, and the new `run_script` function. Previously `sourceFilePath` was only set inside `ModuleLoader::compileModule()`, so relative `import` statements in top-level scripts silently skipped the relative-to-file search path.
`which` argument completion: `which <Tab>` completes program names on `$PATH` (each annotated with the path it resolves to) instead of local file names. `PathCommandIndex` is now the single `$PATH` enumerator, built on the injected `FileSystem` and sharing `CommandResolver::executableExtensions()`, so completion and `which` agree on what counts as executable; `CommandCompleter` consumes it instead of its own `std::filesystem` scan. `which`'s flags are declared once in `whichDescriptor()`, driving the completion spec, the argument parsing, and the `--help` text. `CommandSpecCompleter::isExclusiveFor()` now falls back to top-level `positionalArgs` when a spec has no subcommands, so `which`, `pgrep`, `pidof` and `pkill` no longer mix file paths into their results.
`.endo` file execution from shell: typing `./script.endo` or `path/to/script.endo` at the shell prompt now executes the file in-process (via `executeEndoScript()`) instead of spawning it as an external command. Extension detection in `builtinCallProcess` and `builtinCallProcessShellPiped`. `run_script "path"` F# builtin added for calling scripts from expressions and function bodies (returns exit code as int).
...
- [x] Markdown document rendering in `cat`: on a terminal, `.md` files render as formatted documents rather than syntax-highlighted source. OSC-8 clickable hyperlinks (`TerminalOutput::beginHyperlink`/`endHyperlink`/`writeHyperlink`, absolute URIs only; autolinks and `mailto:` supported, bare URLs deliberately not); DEC double-width/height `#`/`##` titles via `setFullWidthMode(true)`; GitHub-honored centered HTML blocks (`<div|p|h1..h6 align>`, legacy `<center>`, plus inline `<a href>`, `<img>`, `<b>`, `<i>`, `<br>`), with double-width headings padded against a half-width field; inline Sixel images behind an injected `tui::ImageProvider` seam. `--raw` is the single opt-out (also disables highlighting); line-processing flags (`-n`, `-s`, `-r`, …) force the literal source view. Rendered documents carry a configurable left margin (`MarkdownRenderer::setIndent()`, `cat --indent N`, default 1 column) that is subtracted from the content width so centering and table widths account for it. Gating extracted into the pure `chooseCatRenderMode()`. Sixel support probed once via DA1 (`ENDO_SIXEL=0/1` overrides), injected as `SixelCapabilityProvider`. `TerminalOutput::flush()` now writes through a `writeToDestination()` sink so `cat` renders to its own fd, honoring redirects and pipes. — 87 new tests
- [x] Fix nesting-aware markdown link parsing: `findLinkSpan()` in `MarkdownInline.hpp` matches the label's `]` by bracket depth, so the README badge idiom `[![alt](badge.svg)](target)` no longer renders as `![alt](target)`. Shared by `renderInline()` and `stripInlineMarkdown()`, keeping table width math and rendered text in agreement — an autolink or `<a href>` in a table cell now measures at its rendered width instead of its source width.
- [x] Extract `MarkdownHtml.{hpp,cpp}` (tag parser, attribute accessor, align/width parsers, block-tag table, inline-HTML translation), matching the existing `MarkdownInline`/`MarkdownTable` factoring — 33 direct unit tests.
- [x] Fix `resizeImage()` for extreme aspect ratios: a 64×1 banner scaled to 32px wide computed a height of 0 and failed; the auto-computed dimension is now clamped to at least one pixel.
...
- [x] Command palette (Ctrl+Shift+P): `CommandRegistry` data model with `CommandEntry` (id, label, description, category, keybinding hint, context filter, action callback), `CommandPalettePopup` component with fuzzy search filtering via `FuzzyMatch::matchSmartCase()`, keyboard navigation (Up/Down/PgUp/PgDn with wrap), Enter to execute, Escape to dismiss. Integrated into both `PromptComponent` (shell mode) and `AgentInputComponent` (agent mode) with context-aware command filtering (`CommandContext::Shell`/`Agent`/`Both`). HUD overlay detection (DEC mode 2035) for Contour terminal with inline fallback for other terminals.
...
- [x] ~~Add cmake/ctest `CommandSpec` with preset name completion (`CmakeSpec.hpp/cpp`)~~ — **Migrated to endo-scripted completers** (see below). Original C++ specs removed.
- [x] ~~Add ssh/scp `CommandSpec` with host name completion (`SshSpec.hpp/cpp`)~~ — **Migrated to endo-scripted completers** (see below). Original C++ specs removed.
- [x] Fix `startsWith`/`endsWith`/`contains` argument order to data-last convention (`startsWith prefix text`, `endsWith suffix text`, `contains substr text`). Pipeline handler unchanged. Updated stdlib descriptors, documentation, and all test files.
- [x] `register_completer` compile-time function verification: second argument is now a bare identifier (not a string literal), verified at compile time via `lookupFSharpFunction()`. Parser routes `register_completer` to F# mode. IRGenerator validates function exists before emitting native call.
- [x] Fix completer `.endo` scripts: removed `$file` parameter in `$(...)` (cmake/ctest use inline `let-in` bindings), removed `2>/dev/null` redirects (flatpak), replaced awk with grep+split+filter+map (ssh), all use `register_completer "cmd" func_identifier` syntax.
- [x] Endo-scripted completers (`ScriptedCompleter`, `CompleterFunctionRegistry`): Completer functions written in `.endo` scripts, loaded at startup, called at tab-press time. `register_completer <command> <function>` builtin registers a function in the `CompleterFunctionRegistry`. `ScriptedCompleter` (priority 85) looks up registered functions, calls them via stdout capture, applies fuzzy scoring. 2-second TTL cache keyed by `(funcName, args)`. File discovery: `~/.config/endo/completers/` (user overrides, checked first) → `$INSTALL_PREFIX/share/endo/completers/` (bundled); first-found-by-basename wins. Bundled completers: `cmake.endo` (options, generators, configs, preset completion via `Json.query`), `ctest.endo` (options, configs, preset completion), `ssh.endo` (options, host completion via `awk` parsing `~/.ssh/config`), `scp.endo` (options, hosts from `ssh_hosts`), `flatpak.endo` (subcommands, options, dynamic app ID/remote completion). `Json.query` module function added to endo-language for JSON path extraction (`.key` and `[]` syntax, returns `list<string>`, nlohmann/json). — 30 new tests (12 Json.query C++ + 8 Json.query .endo + 5 CompleterFunctionRegistry + 10 ScriptedCompleter), cmake/ssh C++ specs deleted
...
- [x] Buffered completer error display: `BufferingConsoleReport` captures compilation/link errors during tab-completion instead of writing to stderr (which corrupts TUI). Errors are propagated through `CompleterExecutionResult` → `ScriptedCompleter` → `Completer` → `PromptComponent` → displayed above prompt via `Prompt::read()` event loop. Fix: `out.saveCursor()` after writing error lines prevents `flushInline()` from restoring cursor to pre-error position (errors were briefly visible but overwritten by next `draw()`).
- [x] Fix `ssh.endo` type mismatch: `nth 1 parts` returns `option<string>` but was used as plain string. Replaced with `match split " " line with | _ :: host :: _ -> host | _ -> ""` pattern matching. Extracted shared `ssh_hosts` into `_ssh_hosts.endo` (loads before `scp.endo` alphabetically) to fix load-order dependency.
- [x] Completion detail panel (side documentation popup): VS Code-style side panel appearing to the right of the completion popup, showing markdown documentation for the currently selected item. `CompletionItem::detail` field carries markdown content; `StyledText::fromMarkdown()` renders it. Panel uses seamless border merging via `BorderChars` T-junction characters (`topT`/`bottomT`). Detail content cached per selection change, panel width clamped (min 20, max 40 columns), gracefully hidden when terminal is too narrow. Detail populated for all completion kinds: stdlib functions (51 entries with signature and description), keywords (16 entries with syntax examples), builtins (75 entries), constructors, module functions, dot-access record fields, and user-defined symbols. Propagated through `CompletionAdapter`, `FSharpCompleter`, and `PromptComponent` conversion paths. `completionDetail` theme style added. — 9 new tests
- [x] Fuzzy file finder popup (Ctrl+G): Interactive fzf-like file path picker with recursive fuzzy matching (issue #59). `FuzzyPickerPopup` generic TUI component (reusable for any string list) with filter input field, scrollable list, fuzzy match highlighting via `FuzzyMatch::matchSmartCase()`. Scoring prioritizes contiguous substring matches (+200 bonus) and higher coverage percentage (+150 scaled). Uses `ProjectFileTree::filePaths()` for recursive file walking (respects `.gitignore` via `git ls-files`, skips hidden dirs/`build`/`node_modules`, 10k file limit). Integrated into `PromptComponent` as child component (z-index 10) for proper tree-based rendering. Shared utilities extracted: `tui::stringWidth()` (Unicode.hpp/cpp) and `tui::putStringWithHighlights()` (Canvas.hpp/cpp) — deduplicated from 3 popup files. Keybinding pipeline: `EditAction::FuzzyFileFinder` → `InputFieldAction::FuzzyFileFinder` → `PromptComponent` trigger. Selected path shell-escaped via `FileCompleter::escapeForShell()` and inserted at cursor. — 14 new tests
...
- [x] Support OSC-8 hyperlinks in prompts: the working directory is clickable via a `HyperlinkRegion` stored out-of-band on `tui::Buffer` (mirroring `ImageRegion`), so the URI never enters cell text and display-width/hit-testing arithmetic is untouched. `HyperlinkEmitter` opens the framing lazily per written cell, making it correct under the flush diff; the URI is also compared in the diff condition, since `Cell` equality cannot see it and a retargeted link would otherwise never repaint. A gradient-coloured path is one Cell per grapheme yet stays a single logical link. Also clickable: file names in `ls`/`find` output (via a new hidden `FileInfo.path` field) and `grep`'s `filename:line` prefixes (with a `#line` URI fragment). One `shell_hyperlinks` property governs all of it, default on, terminal-only. Shared `platform::fileUri()` also fixes three latent defects in the OSC 7 encoder (locale-dependent `isalnum`, Windows drive letters, UNC paths). — 39 new tests
