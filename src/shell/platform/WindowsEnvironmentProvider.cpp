...
std::optional<std::string_view> WindowsEnvironmentProvider::get(std::string_view name) const
{
    ...
    // Use a thread-local buffer to return a stable string_view
    thread_local std::string buffer;
    buffer.resize(32767); // Max env var size on Windows
...
std::vector<std::string> WindowsEnvironmentProvider::keys() const
{
    ...
    // Collect from OS environment block
    ...
    if (envBlock != nullptr)
    {
        for (auto const* p = envBlock; *p != L'\0';)
        {
            ...
            {
                // Convert wide string key to narrow
                auto const wideKey = entry.substr(0, eq);
                std::string narrowKey;
                narrowKey.reserve(wideKey.size());
                for (auto const wc: wideKey)
                    narrowKey += static_cast<char>(wc); // Safe for ASCII env var names
                result.push_back(std::move(narrowKey));
            }
            p += entry.size() + 1; // Skip past null terminator
        }
        FreeEnvironmentStringsW(envBlock);
    }

    // Merge internal map keys
...
std::expected<void, ShellError> WindowsEnvironmentProvider::changeDirectory(std::filesystem::path const& path)
{
    std::error_code ec;
