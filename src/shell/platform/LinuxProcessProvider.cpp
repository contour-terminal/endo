...
std::vector<ProcessEntry> LinuxProcessProvider::listProcesses() const
{
    ...
    for (auto const& dirEntry: std::filesystem::directory_iterator("/proc", ec))
    {
        ...
        // Parse /proc/[pid]/stat — format: pid (comm) state ppid ... utime stime ...
        ...
        // Extract command name from between parentheses
        ...
        // Parse fields after the comm: state ppid pgrp session tty_nr tpgid flags
        //   minflt cminflt majflt cmajflt utime stime ...
        // Fields are space-separated starting after ") "
        ...
        // Try to get full command line from /proc/[pid]/cmdline
        ...
        if (!cmdlineContents.empty())
        {
            ...
            for (char c: cmdlineContents)
            {
                ...
                {
                    if (!fullCmd.empty())
                        break; // Just use the first argument (program name)
                }
                else
                {
                    fullCmd += c;
