...
std::vector<core::tui::completer::CompletionItem> FilePathCompleter::complete(std::string_view input,
                                                                              size_t cursorPosition)
{
    ...
    for (auto const& filePath: _filePaths)
    {
        ...
        if (core::tui::completer::SmartCaseMatch::matchesPrefix(filePath, query))
        {
            auto const score = core::tui::completer::SmartCaseMatch::adjustScore(100, filePath, query);
            items.push_back(core::tui::completer::CompletionItem {
                .text = fullText,
                .displayText = filePath,
                .description = description,
        ...
        auto const fuzzyResult = core::tui::completer::FuzzyMatch::matchSmartCase(filePath, query);
        if (fuzzyResult.matches)
        {
            auto const score =
                core::tui::completer::FuzzyMatch::calculateScore(50, filePath, query, fuzzyResult);
