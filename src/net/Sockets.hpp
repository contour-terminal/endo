...
/// Binds a TCP listener on @p host : @p port driven by @p runtime's reactor.
...
[[nodiscard]] std::expected<std::unique_ptr<IListener>, NetError> listen(tui::runtime::TuiRuntime& runtime,
...
/// @param runtime The runtime whose reactor drives connect readiness (not owned; a
...
[[nodiscard]] endo::coro::Task<std::expected<std::unique_ptr<ISocket>, NetError>> connect(
    tui::runtime::TuiRuntime* runtime, std::string_view host, std::uint16_t port);
