...
/// `endo::coro::Task<IoResult>`, so it composes with the runtime's coroutine
...
/// tests). All are driven by a @c TuiRuntime: a slow read/write parks the calling
/// coroutine via `runtime.waitReadable/waitWritable` rather than blocking a thread.
...
class ISocket
{
  public:
    ...
    [[nodiscard]] virtual endo::coro::Task<IoResult> read(std::span<std::byte> buffer) = 0;
    ...
    [[nodiscard]] virtual endo::coro::Task<IoResult> write(std::span<std::byte const> buffer) = 0;
