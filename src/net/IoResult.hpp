...
/// Classifies a network failure. Mirrors the categories the shell, agent, and the
/// `httpServe` builtin need to distinguish (clean shutdown vs cancellation vs a
...
struct NetError
{
    ...
    {
        auto result = std::string { endo::net::toString(code) };
        if (!context.empty())
            result += " (" + context + ")";
        if (systemCode != 0)
