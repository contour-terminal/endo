...
endo::coro::Task<IoResult> PosixSocket::read(std::span<std::byte> buffer)
{
    while (true)
    {
        if (_closed || _fd < 0)
...
endo::coro::Task<IoResult> PosixSocket::write(std::span<std::byte const> buffer)
{
    std::size_t total = 0;
    while (total < buffer.size())
    {
        ...
        if (isWouldBlock(err))
        {
            co_await _runtime.waitWritable(_fd);
            continue;
        }
        if (err == EINTR)
            continue;
