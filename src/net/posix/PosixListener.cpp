...
PosixListener::PosixListener(tui::runtime::TuiRuntime& runtime, int fd, std::uint16_t localPort) noexcept:
    _runtime(runtime), _fd(fd), _localPort(localPort)
{
}
...
std::expected<std::unique_ptr<PosixListener>, NetError> PosixListener::bind(tui::runtime::TuiRuntime& runtime,
                                                                            ...
                                                                            int backlog)
{
    ...
    return std::unique_ptr<PosixListener>(new PosixListener(runtime, fd, actualPort));
}

endo::coro::Task<AcceptResult> PosixListener::accept()
{
    while (true)
    {
        if (_closed || _fd < 0)
    ...
    #endif
            co_return std::unique_ptr<ISocket>(new PosixSocket(_runtime, conn, formatPeer(peer)));
        }
        ...
        if (err == EAGAIN || err == EWOULDBLOCK)
        {
            ...
            try
            {
                co_await _runtime.waitReadable(_fd);
            }
