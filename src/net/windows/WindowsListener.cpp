...
WindowsListener::WindowsListener(tui::runtime::TuiRuntime& runtime,
                                 SOCKET socket,
                                 WSAEVENT event,
                                 std::uint16_t localPort) noexcept:
    _runtime(runtime), _socket(socket), _event(event), _localPort(localPort)
{
}
...
std::expected<std::unique_ptr<WindowsListener>, NetError> WindowsListener::bind(
    tui::runtime::TuiRuntime& runtime, std::string_view host, std::uint16_t port, int backlog)
{
    auto hints = addrinfo {};
    ...
    return std::unique_ptr<WindowsListener>(new WindowsListener(runtime, sock, event, actualPort));
}

endo::coro::Task<AcceptResult> WindowsListener::accept()
{
    while (true)
    {
        ...
        if (conn != INVALID_SOCKET)
            co_return std::unique_ptr<ISocket>(new WindowsSocket(_runtime, conn, formatPeer(peer)));
