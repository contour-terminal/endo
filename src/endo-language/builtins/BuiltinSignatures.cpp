...
namespace
{
    ...
    constexpr std::array shellBuiltinDescriptors = {
        ...
        ShellBuiltinDescriptor { .name="hostname", .description="builtin", .detail="**hostname** -- builtin\n\nPrint the system hostname." },
        ShellBuiltinDescriptor { .name="date", .description="builtin", .detail="**date** -- builtin\n\nPrint or format the current date and time.\n\n```\ndate\ndate --epoch\ndate --iso\ndate +%Y-%m-%d\n```" },
        ShellBuiltinDescriptor { .name="uname", .description="builtin", .detail="**uname** -- builtin\n\nPrint system information.\n\n```\nuname\nuname -a\nuname -s -m\n```" },
        ShellBuiltinDescriptor { .name="basename", .description="builtin", .detail="**basename** -- builtin\n\nStrip directory from pathname.\n\n```\nbasename /usr/local/file.txt\nbasename /usr/local/file.txt .txt\n```" },
        ShellBuiltinDescriptor { .name="dirname", .description="builtin", .detail="**dirname** -- builtin\n\nStrip last component from pathname.\n\n```\ndirname /usr/local/file.txt\n```" },
        ShellBuiltinDescriptor { .name="realpath", .description="builtin", .detail="**realpath** -- builtin\n\nResolve to absolute canonical path.\n\n```\nrealpath ./file.txt\n```" },
        ShellBuiltinDescriptor { .name="touch", .description="builtin", .detail="**touch** -- builtin\n\nCreate file or update timestamps.\n\n```\ntouch file.txt\ntouch -c existing.txt\n```" },
        ShellBuiltinDescriptor { .name="ln", .description="builtin", .detail="**ln** -- builtin\n\nCreate hard or symbolic links.\n\n```\nln -s target link\nln file hardlink\n```" },
        ShellBuiltinDescriptor { .name="mktemp", .description="builtin", .detail="**mktemp** -- builtin\n\nCreate a temporary file or directory.\n\n```\nmktemp\nmktemp -d\n```" },
        ShellBuiltinDescriptor { .name="head", .description="builtin", .detail="**head** -- builtin\n\nOutput the first lines of files.\n\n```\nhead file.txt\nhead -n 5 file.txt\necho \"data\" | head -n 1\n```" },
        ShellBuiltinDescriptor { .name="tail", .description="builtin", .detail="**tail** -- builtin\n\nOutput the last lines of files.\n\n```\ntail file.txt\ntail -n 5 file.txt\necho \"data\" | tail -n 1\n```" },
        ShellBuiltinDescriptor { .name="wc", .description="builtin", .detail="**wc** -- builtin\n\nCount lines, words, and characters.\n\n```\nwc file.txt\necho \"hello world\" | wc -w\n```" },
        ShellBuiltinDescriptor { .name="sort", .description="builtin", .detail="**sort** -- builtin\n\nSort lines of text.\n\n```\nsort file.txt\necho \"b\\na\" | sort\nsort -rn data.txt\n```" },
        ShellBuiltinDescriptor { .name="uniq", .description="builtin", .detail="**uniq** -- builtin\n\nFilter adjacent duplicate lines.\n\n```\nsort file.txt | uniq\nuniq -c file.txt\n```" },
