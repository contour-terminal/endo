...
/// @file Clock.hpp
/// @brief `IClock` — the monotonic-clock dependency-injection seam.
///
/// Production code uses @c SteadyClock; tests inject @c ManualClock so timer,
/// timeout, and `sleep` behaviour is deterministic without real wall-clock
/// waits. Mirrors the @c ProcessProvider / @c EnvironmentProvider DI pattern:
/// an abstract interface, a native implementation, and a test double.
///
/// Every component that schedules against a deadline — the TUI runtime, the
/// reactor's timer heap, the (future) async socket timeouts — takes an
/// @c IClock by reference rather than calling @c std::chrono::steady_clock::now()
/// directly, so its time source is controllable in tests.
...
/// Monotonic time point used for every deadline and timeout in endo. Wall-clock
/// time is never used for internal scheduling, so system-clock skew (NTP
/// adjustments, manual changes) cannot perturb timer semantics. Aliased to
/// @c std::chrono::steady_clock::time_point so existing deadline fields need no
/// type churn when migrated onto @c IClock.
...
/// default argument of a runtime constructor). Prefer explicit constructor
